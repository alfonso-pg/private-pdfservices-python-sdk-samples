...
The sample class ```scan_pdf_for_pii.py``` extracts text and coordinates from a PDF using the ExtractPDF operation, then runs a regex-based PII (Personally Identifiable Information) scanner to detect SSN, email, phone numbers, credit cards, and IBANs. Output is a JSON report with masked findings and location metadata.

```$xslt
python src/piiscan/scan_pdf_for_pii.py
```

Optionally pass an input path: `python src/piiscan/scan_pdf_for_pii.py path/to/your.pdf`
